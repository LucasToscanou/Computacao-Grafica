...
float* Disk::generateDiskVertices() {
    ...
    float* vertices = new float[(m_nslice + 2) * 4];
    ...
    for (int i = 0; i < m_nslice; ++i) {
        ...
        float u = 0.5f + 0.5f * cos(angle); // Map to [0,1] range
        float v = 0.5f + 0.5f * sin(angle); // Map to [0,1] range

        vertices[(i + 1) * 4]     = x; // x position
        vertices[(i + 1) * 4 + 1] = y; // y position
        vertices[(i + 1) * 4 + 2] = u; // u (texture)
        vertices[(i + 1) * 4 + 3] = v; // v (texture)
    }

    // Add the first perimeter vertex again at the end to close the fan
    vertices[(m_nslice + 1) * 4]     = vertices[4];  // x of first perimeter vertex
    vertices[(m_nslice + 1) * 4 + 1] = vertices[5];  // y of first perimeter vertex
    vertices[(m_nslice + 1) * 4 + 2] = vertices[6];  // u of first perimeter vertex
    vertices[(m_nslice + 1) * 4 + 3] = vertices[7];  // v of first perimeter vertex
